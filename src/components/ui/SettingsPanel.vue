...
<template>
  <Teleport to="body">
    <div v-if="settingsStore.showHelp" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @click="close">
      <div class="bg-white dark:bg-culinary-dark rounded-3xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto" @click.stop>
        <div class="p-8">
          ...
          <div class="space-y-6">
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <span :class="settingsStore.soundEnabled ? 'text-culinary-accent' : 'text-slate-400'">
            ...
            <div class="space-y-3">
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center">
                  <div class="text-2xl font-black text-culinary-primary">{{ statsStore.totalSearches }}</div>
                  <div class="text-xs text-slate-500 dark:text-slate-400">Searches</div>
                </div>
                <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center">
                  <div class="text-2xl font-black text-culinary-secondary">{{ statsStore.totalRecipesViewed }}</div>
                  <div class="text-xs text-slate-500 dark:text-slate-400">Recipes Viewed</div>
                </div>
                <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center">
                  <div class="text-2xl font-black text-culinary-primary">{{ statsStore.formatTime() }}</div>
                  <div class="text-xs text-slate-500 dark:text-slate-400">Time Spent</div>
                </div>
                <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center">
                  <div class="text-2xl font-black text-culinary-secondary">{{ statsStore.totalFavorites }}</div>
